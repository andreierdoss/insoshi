class ConnectionsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      ...
      when t('connection.accept')
        @connection.accept
        flash[:notice] = t('flash.accepted_connection', :person_link => person_url(contact), :person_name => name)
      when t('connection.decline')
        @connection.breakup
        flash[:notice] = t('flash.declined_connection', :person_name => name)
      end
      format.html { redirect_to(home_url) }
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      flash[:success] = t('flash.ended_connection', :person_name => @connection.contact.name)
